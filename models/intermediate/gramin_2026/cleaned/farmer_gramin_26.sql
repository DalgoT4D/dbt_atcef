{{ config(
  materialized='table',
  tags=["intermediate","intermediate_gramin_niti", "gramin_niti", "gramin_26"]
) }}
...
with farmers as (
    ...
    from
        {{ source('source_gramin_26', 'subjects_gramin_26') }} as subjects
    left join
        {{ ref('address_gramin_26') }} as rwb
        on
...
approval_farmers as (
    ...
    from farmers as d
    inner join {{ ref('approval_status_gramin_26') }} as a
        on
