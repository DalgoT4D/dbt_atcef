{{ config(
  materialized='table',
  tags=["intermediate","intermediate_gramin", "gramin_niti", "gramin_26"]
) }}
...
WITH WORKORDERENCOUNTERS AS (
    ...
    FROM {{ ref('encounters_gramin_26') }} AS E
    LEFT JOIN {{ ref('subjects_gramin_26') }} AS S
        ON
            E.SUBJECT_ID = S.UID
            AND E.ENCOUNTER_TYPE <> 'Work order daily Recording - Farmer'
    LEFT JOIN {{ ref('subjects_gramin_26') }} AS F
        ON
...
FROM {{ ref('subjects_gramin_26') }} AS S
