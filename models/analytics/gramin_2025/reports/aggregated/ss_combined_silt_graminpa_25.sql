...
-- for GraminPA 2025 using only analytics lineage sources.
{{ config(
  materialized='table',
...
farmer_silt_by_wo AS (
    ...
    FROM {{ ref('daily_farmer_linelist_graminpa_25') }} AS d
    LEFT JOIN (
        SELECT DISTINCT
            farmer_beneficiary_id,
            COALESCE(area_silt_spread, 0) AS area_silt_spread
        FROM {{ ref('farmer_endline_linelist_graminpa_25') }}
    ) AS fe
...
gp_silt AS (
    SELECT
        ...
        gp.last_gp_update
    FROM {{ ref('gp_linelist_approval_graminpa_25') }} AS gp
