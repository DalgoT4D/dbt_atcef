-- Keeps the latest farmer_endline_graminpa_25 per farmer, adds registration fields, 
...
WITH farmer_latest_records AS (
    ...
    FROM {{ ref('farmer_endline_graminpa_25') }} AS t1 
...
farmer_with_status AS (
    ...
    LEFT JOIN {{ ref('approval_status_graminpa_25') }} AS asn
...
LEFT JOIN {{ ref('farmer_regn_graminpa_25') }} AS fr
