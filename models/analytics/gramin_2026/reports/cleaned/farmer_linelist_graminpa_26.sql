...
-- Joins farmer_regn_graminpa_26 with carting totals and latest endline info
-- without filtering  approval outcome.
{{ config(
  materialized='table',
  tags=["analytics", "graminpa_2026", "graminpa", "analytical_models", "reports_graminpa_2026"]
) }}
...
with farmer_totals as (
  ...
  FROM {{ ref('daily_farmer_linelist_graminpa_26') }} AS w
...
latest_endline AS (
    ...
    FROM {{ ref('farmer_endline_graminpa_26') }}
    WHERE
...
SELECT
  ...
  w.total_silt_carted
    ...
    FROM {{ ref('farmer_regn_graminpa_26') }} AS s
