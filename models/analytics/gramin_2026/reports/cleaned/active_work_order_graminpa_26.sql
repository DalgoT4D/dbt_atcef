...
-- Work-order-level analytical table for GraminPA 2026. Farmer and machine
...
with carting_by_machine_type AS (
    ...
    FROM {{ ref('daily_farmer_linelist_graminpa_26') }} AS df
...
FROM {{ ref('daily_machine_linelist_graminpa_26') }} as dm
LEFT JOIN {{ ref('machine_regn_graminpa_26') }} AS mr
...
LEFT JOIN {{ ref('machine_endline_linelist_graminpa_26') }} me
...
--     FROM {{ ref('work_order_farmer_graminpa_26') }} AS wf
--     INNER JOIN {{ ref('approval_status_graminpa_26') }} AS a
...
-- GP SILT
   gp_silt as( SELECT
    ...
    from {{ ref('gp_endline_graminpa_26') }} as ge
    INNER JOIN {{ ref('approval_status_graminpa_26') }} as a 
...
LEFT JOIN {{ref('workorder_endline_linelist_graminpa_26')}} AS we ON wd.workorderid = we.workorder_id
