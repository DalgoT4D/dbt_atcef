-- Machine registration table with cleaned identifiers, location, and approval metadata for graminpa 2026.
{{ config(
  materialized='table',
...
-- FROM 
-- {{ ref('dim_subjects_machine_graminpa_26') }} AS m
-- LEFT JOIN 
...
WITH m_ranked AS (
    ...
    FROM {{ ref('dim_subjects_machine_graminpa_26') }} m
    WHERE m.voided != TRUE
),
...
LEFT JOIN (
    SELECT DISTINCT ON (entity_id)
        ...
        approval_status,
        status_date_time
    FROM {{ ref('approval_status_graminpa_26') }}
