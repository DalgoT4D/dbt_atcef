...
-- ANALYTICAL TABLE: Work Orders × Farmers × Machines × Silt (NITI 2024)
--
-- ------------------------------------------------------------
...
--      - daily_machine_linelist_niti_24
--      - machine_regn_niti_24
--      - machine_endline_linelist_niti_24
--
...
--   - farmer_silt_cl
--   - gp_silt
--   - workorder_endline_linelist_niti_24
--   - machine_silt
...
with carting_by_machine_type AS (
    ...
    FROM {{ ref('daily_farmer_linelist_niti_24') }} AS df
...
FROM {{ ref('daily_machine_linelist_niti_24') }} as dm
LEFT JOIN {{ ref('machine_regn_niti_24') }} AS mr
...
LEFT JOIN {{ ref('machine_endline_linelist_niti_24') }} me
...
approved_work_orders AS (
    ...
    FROM {{ ref('work_order_regn_niti_24') }} AS w
...
farmer_silt_cl as (
    ...
    from {{ ref('daily_farmer_linelist_niti_24') }}
...
-- GP SILT
   gp_silt as( SELECT
    ...
    from {{ ref('gp_endline_niti_24') }} as ge
    INNER JOIN {{ ref('approval_status_niti_24') }} as a 
...
LEFT JOIN {{ref('workorder_endline_linelist_niti_24')}} AS we ON wd.workorderid = we.workorder_id
