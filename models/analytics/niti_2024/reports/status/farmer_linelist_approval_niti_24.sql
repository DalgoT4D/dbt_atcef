-- Joins farmer_regn_niti_24 with carting totals and latest endline info
...
with farmer_totals as (
  ...
  FROM {{ ref('daily_farmer_linelist_approval_niti_24') }} AS w
...
latest_endline AS (
    ...
    FROM {{ ref('farmer_endline_niti_24') }}
    WHERE
...
SELECT
  ...
  w.total_silt_carted
    ...
    FROM {{ ref('farmer_regn_niti_24') }} AS s
