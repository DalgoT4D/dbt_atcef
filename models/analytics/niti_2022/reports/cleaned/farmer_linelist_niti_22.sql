...
-- SELECT *
-- FROM {{ ref('farmer_linelist_approval_niti_22') }}
-- WHERE approval_status = 'Approved'


-- Joins farmer_regn_niti_22 with carting totals and latest endline info
...
with farmer_totals as (
  ...
  FROM {{ ref('daily_farmer_linelist_niti_22') }} AS w
...
latest_endline AS (
    ...
    FROM {{ ref('farmer_endline_niti_22') }}
    WHERE
...
SELECT
  ...
  w.total_silt_carted
    ...
    LEFT JOIN {{ ref('farmer_regn_niti_22') }} AS s
