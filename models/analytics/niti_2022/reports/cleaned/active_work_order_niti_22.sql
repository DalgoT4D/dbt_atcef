...
-- ANALYTICAL TABLE: Work Orders × Farmers × Machines × Silt (NITI 2022)
--
-- ------------------------------------------------------------
...
--      - daily_machine_linelist_niti_22
--      - machine_regn_niti_22
--      - machine_endline_linelist_niti_22
--
...
--   - farmer_silt_cl
--   - gp_silt
--   - workorder_endline_linelist_niti_22
--   - machine_silt
...
with carting_by_machine_type AS (
    ...
    FROM {{ ref('daily_farmer_linelist_niti_22') }} AS df
    LEFT JOIN {{ ref('machine_regn_niti_22') }} AS mr
...
FROM {{ ref('daily_machine_linelist_niti_22') }} as dm
LEFT JOIN {{ ref('machine_regn_niti_22') }} AS mr
...
LEFT JOIN {{ ref('machine_endline_linelist_niti_22') }} me
...
approved_work_orders AS (
    ...
    FROM {{ ref('work_order_regn_niti_22') }} AS w
...
farmer_silt_cl as (
    ...
    from {{ ref('daily_farmer_linelist_niti_22') }}
...
-- GP SILT
   gp_silt as( SELECT
    ...
    from {{ ref('gp_endline_niti_22') }} as ge
    INNER JOIN {{ ref('approval_status_niti_22') }} as a 
...
LEFT JOIN {{ref('workorder_endline_linelist_niti_22')}} AS we ON wd.workorderid = we.workorder_id
