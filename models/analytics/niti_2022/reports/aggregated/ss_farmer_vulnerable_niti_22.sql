-- Farmer vulnerability split for NITI 2022 using distinct farmers from the
-- approved daily farmer linelist and registration categories.
{{ config(
  materialized='table',
  tags=["analytics", "analytics_niti_2022", "analytical_models", "reports_niti_2022", "ss_niti_2022"]
) }}
...
farmer_counts AS (
    SELECT
        ...
        COUNT(CASE WHEN fr.farmer_category IN ('Semi-medium (5-9.99 acres)', 'Medium (10-24.99 acres)', 'Large (above 25 acres)', 'Large (25+ acres)') THEN fr.subject_id END) AS other_farmers,
        ...
        SUM(CASE WHEN fr.farmer_category IN ('Semi-medium (5-9.99 acres)', 'Medium (10-24.99 acres)', 'Large (above 25 acres)', 'Large (25+ acres)') THEN COALESCE(d.total_silt_carted, 0) ELSE 0 END) AS other_silt
    FROM farmer_activity AS d
    INNER JOIN {{ ref('farmer_regn_niti_22') }} AS fr
