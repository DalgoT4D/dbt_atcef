-- Machine efficiency metrics for NITI 2022 using analytics lineage only.
{{ config(
  materialized='table',
...
efficiency AS (
    SELECT
        ...
        ROUND(CAST(COALESCE(s.total_silt_carted, 0) AS NUMERIC)
            / NULLIF(CAST(COALESCE(w.total_working_hours, 0) AS NUMERIC), 0), 2) AS avg_silt_excavated_per_hour
...
machine_details AS (
    ...
    FROM {{ ref('machine_regn_niti_22') }} AS m
