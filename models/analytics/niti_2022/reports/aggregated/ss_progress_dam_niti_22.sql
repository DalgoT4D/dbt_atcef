-- Work order progress status for NITI 2022 using only analytics lineage.
{{ config(
  materialized='table',
...
latest_workorder_endline AS (
    ...
    FROM {{ ref('workorder_endline_linelist_niti_22') }} AS we
...
latest_farmer_activity AS (
    ...
    FROM {{ ref('daily_farmer_linelist_niti_22') }} AS d
