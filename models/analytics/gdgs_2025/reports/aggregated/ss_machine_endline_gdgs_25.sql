-- Machine data with machine endline status for GDGS 2025 using analytics
...
WITH machines_with_daily_recording AS (
    SELECT DISTINCT
        machine_id
    FROM {{ ref('daily_machine_linelist_gdgs_25') }}
...
approved_machines AS (
    ...
    FROM {{ ref('machine_regn_gdgs_25') }} AS m
...
machine_endline_exists AS (
    SELECT DISTINCT
        machine_id
    FROM {{ ref('machine_endline_linelist_gdgs_25') }}
    WHERE machine_id IS NOT NULL
)
