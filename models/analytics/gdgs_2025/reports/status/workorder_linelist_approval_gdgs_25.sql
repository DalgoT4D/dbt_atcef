...
WITH machine_contractors AS (
    ...
    FROM {{ ref('work_order_machine_gdgs_25') }} AS wm
...
from
{{ ref('work_order_regn_gdgs_25') }} as ws
left join
{{ ref('work_order_endline_gdgs_25') }} as we -- only take the latest instance of work_order_endline - to be coded in later
on
