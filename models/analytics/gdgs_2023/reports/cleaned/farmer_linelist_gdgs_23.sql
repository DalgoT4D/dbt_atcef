...
with farmer_totals as (
  ...
  FROM {{ ref('daily_farmer_linelist_gdgs_23') }} AS w
...
latest_endline AS (
    SELECT
        *,
        ...
        ROW_NUMBER() OVER (
            PARTITION BY farmer_beneficiary_id
            ORDER BY encounter_date_time DESC
        ) AS rn
    FROM {{ ref('farmer_endline_linelist_gdgs_23') }}
)
...
SELECT
  ...
  w.total_silt_carted
    ...
    LEFT JOIN {{ ref('farmer_regn_gdgs_23') }} AS s
    ON w.farmer_beneficiary_id = s.subject_id

    LEFT JOIN latest_endline AS fe
    ON s.subject_id = fe.farmer_beneficiary_id
    AND fe.rn = 1
