-- Joins farmer_regn_gdgs_23 with carting totals and latest endline info
...
with farmer_totals as (
  ...
  FROM {{ ref('daily_farmer_linelist_approval_gdgs_23') }} AS w
...
latest_endline AS (
    ...
    FROM {{ ref('farmer_endline_analytics_gdgs_23') }}
    WHERE
...
SELECT
  ...
  w.total_silt_carted
    ...
    FROM {{ ref('farmer_regn_gdgs_23') }} AS s
