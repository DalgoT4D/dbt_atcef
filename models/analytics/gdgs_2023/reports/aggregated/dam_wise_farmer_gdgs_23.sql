...
{{ config(
  materialized='table',
  tags=["analytics","analytics_gdgs_2023", "analytical_models", "reports_gdgs_2023", "aggregated_gdgs_23"]
) }}
...
WITH daily_summed AS (
    ...
    FROM {{ ref('daily_farmer_linelist_gdgs_23') }} AS d
    LEFT JOIN {{ ref('farmer_regn_gdgs_23') }} AS fc
...
SELECT
    ...
    COUNT(Case when de.farmer_category = 'Large (above 24 acres)' THEN de.farmer_id END) AS large_farmers,
    ...
    SUM(CASE WHEN de.farmer_category = 'Large (above 24 acres)' THEN de.silt_carted ELSE 0 END) AS large_farmers_silt_carted,
    ...
    SUM(CASE WHEN de.farmer_category = 'Family of farmer who committed suicide' THEN de.silt_carted ELSE 0 END) AS family_members_suicide_farmers_silt_carted,
        ...
        SUM(CASE WHEN de.farmer_category = 'Large (above 24 acres)' THEN de.amt_silt_used_non_farm_purpose ELSE 0 END) AS large_farmers_silt_for_non_farm_purpose,
    ...
    LEFT JOIN {{ ref('farmer_endline_linelist_gdgs_23') }} AS fe
