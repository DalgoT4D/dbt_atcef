...
WITH daily_summed AS (
    SELECT
    fc.farmer_name,
    fc.subject_id as farmer_id, -- new
    fc.state,
    fc.district,
    fc.taluka,
    fc.village, 
    ...
    FROM {{ ref('farmer_regn_niti_23') }} AS fc
    LEFT JOIN {{ ref('daily_farmer_linelist_niti_23') }} AS d
...
SUM(CASE WHEN de.farmer_category = 'Family of farmer who committed suicide' THEN COALESCE(de.amt_silt_used_non_farm_purpose, 0) ELSE 0 END) AS family_members_suicide_farmers_silt_for_non_farm_purpose,
    ...
    LEFT JOIN {{ ref('farmer_endline_linelist_niti_23') }} AS fe
