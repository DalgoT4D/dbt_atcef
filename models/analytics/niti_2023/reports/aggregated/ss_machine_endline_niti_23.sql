-- Machine data with machine endline status for NITI 2023 using analytics
...
WITH machines_with_daily_recording AS (
    SELECT DISTINCT
        machine_id
    FROM {{ ref('daily_machine_linelist_niti_23') }}
...
machine_endline_exists AS (
    SELECT DISTINCT
        machine_id
    FROM {{ ref('machine_endline_linelist_niti_23') }}
    WHERE machine_id IS NOT NULL
)
