-- Keeps the latest approval status per entity from the gdgs_2024 approval_statuses source 
-- so downstream models can read a single current status flag.

{{ config(
  materialized='table',
  tags=["analytics", "analytics_gdgs_2024", "analytics_intermediate", "source_cleaned_gdgs_2024"]
) }}
...
WITH RankedStatuses AS (
    ...
    FROM
        {{ source('source_gdgsom_surveys', 'approval_statuses') }}
)
