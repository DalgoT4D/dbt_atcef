...
WITH entity_statuses AS (
    SELECT 'Farmer Registration' AS entity, 
        ...
        approval_status
    FROM {{ ref('farmer_linelist_approval_gdgs_24') }}
    ...
    SELECT 'Machine Endline' AS entity, 
        state, district, taluka, village, dam, --stakeholder_responsible,
        approval_status
    FROM {{ ref('machine_endline_linelist_approval_gdgs_24') }}
    ...
    SELECT 'Work order registration' AS entity,
        state, district, taluka, village, dam, --stakeholder_responsible,
        approval_status
    FROM {{ ref('workorder_linelist_approval_gdgs_24') }}
    ...
    FROM {{ ref('daily_farmer_linelist_approval_gdgs_24') }}
