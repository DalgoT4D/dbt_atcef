...
{{ config(
  materialized='table',
  tags=["analytics","analytics_gdgs_2024", "analytical_models", "reports_gdgs_2024", "aggregated_gdgs_24"]
) }}
...
WITH daily_summed AS (
    ...
    FROM {{ ref('daily_farmer_linelist_gdgs_24') }} AS d
    LEFT JOIN {{ ref('farmer_regn_gdgs_24') }} AS fc
...
SELECT
    ...
    LEFT JOIN {{ ref('farmer_endline_linelist_gdgs_24') }} AS fe
