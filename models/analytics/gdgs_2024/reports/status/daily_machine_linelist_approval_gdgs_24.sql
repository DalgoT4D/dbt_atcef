-- Lists machine encounters by joining work_order_machine_gdgs_24 to machine registrations and approvals
...
WITH machine_logs AS (
    SELECT
        ...
        w.working_hours
    FROM {{ ref('work_order_machine_gdgs_24') }} AS w
    WHERE w.voided != TRUE
)
...
INNER JOIN {{ ref('work_order_regn_gdgs_24')}} AS wr
