-- Keeps the latest farmer_endline_gdgs_24 per farmer, adds registration fields, 
...
WITH farmer_latest_records AS (
    ...
    FROM {{ ref('farmer_endline_analytics_gdgs_24') }} AS t1 
...
LEFT JOIN {{ ref('farmer_regn_gdgs_24') }} AS fr
    ON fe.endline_farmer_sub_id = fr.subject_id

LEFT JOIN {{ ref('approval_status_gdgs_24') }} AS asn
