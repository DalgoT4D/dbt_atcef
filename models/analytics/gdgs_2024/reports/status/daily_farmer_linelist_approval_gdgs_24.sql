...
WITH non_voided_work_orders AS (
    SELECT
        *
    FROM {{ ref('work_order_farmer_gdgs_24') }}
...
FROM {{ ref('farmer_regn_gdgs_24') }} AS fs
LEFT JOIN non_voided_work_orders AS w
    ON fs.subject_id = w.farmer_beneficiary_id
LEFT JOIN {{ ref('work_order_regn_gdgs_24') }} AS ws   -- Join on the work order subject ID
    ON w.farmer_work_order_sub_id = ws.subject_id
LEFT JOIN {{ref('approval_status_gdgs_24')}} AS a   -- Join on the encounter/event ID (eid) for approval status
