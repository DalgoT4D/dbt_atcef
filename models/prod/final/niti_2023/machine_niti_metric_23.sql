...
WITH working_hours AS (
    ...
    FROM {{ ref('machine_niti_2023_agg') }} AS e
...
silt_carted AS (
    ...
    FROM {{ ref('farmer_silt_calc_niti_2023') }} AS e
    GROUP BY e.machine_sub_id
),
...
benchmark AS (
    ...
    LEFT JOIN {{ ref('machine_niti_2023_agg') }} AS m
...
final AS (
    ...
    FROM benchmark AS b
    INNER JOIN {{ ref('machine_niti_2023_agg') }} AS m
