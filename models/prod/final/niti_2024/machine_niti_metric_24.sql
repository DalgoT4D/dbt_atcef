...
WITH working_hours AS (
    ...
    FROM {{ ref('machine_niti_2024_agg') }} AS e
...
silt_carted AS (
    ...
    FROM {{ ref('farmer_calc_silt_niti_2024') }} AS e
    GROUP BY e.machine_sub_id
),
...
benchmark AS (
    ...
    LEFT JOIN {{ ref('machine_niti_2024_agg') }} AS m
...
final AS (
    ...
    FROM benchmark AS b
    INNER JOIN {{ ref('machine_niti_2024_agg') }} AS m
