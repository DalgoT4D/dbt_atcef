...
WITH working_hours AS (
    ...
    FROM {{ref('machine_niti_2024_agg')}} e
...
silt_carted AS (
    ...
    FROM {{ref('farmer_calc_silt_niti_2024')}} e
    GROUP BY e.machine_sub_id
),
...
benchmark AS (
    ...
    FROM efficiency e
    LEFT JOIN {{ref('machine_niti_2024_agg')}} m 
...
final AS (
    ...
    FROM benchmark b
    JOIN {{ref('machine_niti_2024_agg')}} m 
        ON b.machine_id = m.machine_sub_id
    WHERE m.machine_voided = FALSE
