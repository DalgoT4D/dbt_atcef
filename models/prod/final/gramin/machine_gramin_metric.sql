...
WITH working_hours AS (
    ...
    FROM {{ ref('machine_gramin_aggregated') }} AS e
...
silt_carted AS (
    ...
    FROM {{ ref('farmer_calc_silt_gramin') }} AS e
    GROUP BY e.machine_sub_id
),
...
benchmark AS (
    ...
    LEFT JOIN {{ ref('machine_gramin_aggregated') }} AS m
...
final AS (
    ...
    FROM benchmark AS b
    INNER JOIN {{ ref('machine_gramin_aggregated') }} AS m
